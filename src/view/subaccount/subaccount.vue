    ...
    <Split v-model="split2" mode="vertical" min="300px" max="300px" style="height:100%;width:100%;">
      <div slot="top" class="accounts-split-pane-top">
        ...
        <div class="accounts-table-area" style="height:100%">
          <Table :columns="basicColumns" show-header strip :border="true" :data="accountsList" :loading="loadingFlag" @on-row-click="selectOneAccount"></Table>
        </div>
      ...
      <div slot="bottom" class="accounts-split-pane-bottom">
        <Tabs type="card">
          <TabPane label="描述" icon="ios-funnel">
            <Table :columns="columns1" show-header strip :border="true" :data="selectedAccount" row-key="id"></Table>
          </TabPane>
          <TabPane label="用户" icon="ios-people">
            <Users />
          </TabPane>
          <TabPane label="账户设置" icon="ios-cog">
            <Icon type="ios-cog" />标签三的内容
          </TabPane>
        </Tabs>
      </div>
    </Split>
    <!-- 添加账户对话框 -->
    <Modal v-model="addAccountModal" draggable sticky scrollable :mask="false" title="添加账户" class="my-modal" @on-ok="submitAddAccountForm" :closable="false" @on-cancel="closeAddAccountModal">
...
export default {
  data() {
    ...
    return {
      ...
      addAccountFormRules: {
        ...
        organizationEmail: [
          { required: true, message: 'The email cannot be empty', trigger: 'blur', validator: checkEmail }
  ...
  methods: {
    ...
    async submitAddAccountForm() {
      this.$refs.addAccountForm.validate(
        async (valid) => {
          if (!valid) {
    ...
    formatSelectedAccount(data) {
      var result = [{ id: '10', name: 'account info', value: '', children: [] }, { id: '11', name: "organizition info", value: "", children: [] }]
      var index = 1
      for (var i in data) {
